...
export const authorizeTeacherMiddlewareDelete = async (req: Request, res: Response, next: NextFunction) => {
    const userId = req.user?.id; // ID del usuario autenticado
    const resourceId = parseInt(req.params.id, 10); // ID de la sección desde los parámetros de la ruta
    ...
    try {
        ...
        // Verificar si el teacherId en la sección coincide con el userId autenticado
        if (section.teacherId !== userId) {
            return res.status(403).json({ error: 'El usuario no tiene acceso a esta sección Y rescurso.' });
        }
